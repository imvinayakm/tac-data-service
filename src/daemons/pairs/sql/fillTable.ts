...
const selectPairsCTE = pg
  .with('pairs_cte', qp => {
    qp.select({
      ...
      volume_waves: pg.raw(
        "case when amount_asset = 'TAC' then sum(e.amount * 10 ^(-a_dec.decimals)) when price_asset = 'TAC' then sum(e.amount * 10 ^(-a_dec.decimals) * e.price * 10 ^(-8 - p_dec.decimals + a_dec.decimals)) end"
      ),
